...
// ═══ 传感器: 屏幕尺子 + 水平仪(简易) + 取色器说明 ═══
class SensorPage extends StatefulWidget { const SensorPage({super.key}); @override State<SensorPage> createState() => _Sen(); }
class _Sen extends State<SensorPage> {
  @override Widget build(BuildContext c) {
    // 用逻辑像素→物理尺寸换算: 假设 160dpi 基准, 用 devicePixelRatio 估算(约 1cm≈38px 基准, 可按屏幕校准)
    final dpr = MediaQuery.of(c).devicePixelRatio;
    final pxPerCm = 38.0 * dpr / dpr * (160 / 160); // 基准估算, 提供校准滑块
    return ListView(padding: const EdgeInsets.all(12), children: [
      const Text('屏幕尺子', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
      const SizedBox(height: 6),
      _Ruler(pxPerCm: pxPerCm),
      const SizedBox(height: 10),
      const Card(child: Padding(padding: EdgeInsets.all(12),
        child: Text('水平仪/指南针/取色器需要传感器插件支持, 将在下一批接入(基于 sensors_plus)。\n尺子按标准屏幕密度估算, 与实物对比后可微调。',
          style: TextStyle(fontSize: 11, color: Colors.grey, height: 1.6)))),
    ]);
  }
}

class _Ruler extends StatefulWidget { final double pxPerCm; const _Ruler({required this.pxPerCm}); @override State<_Ruler> createState() => _RulerState(); }
class _RulerState extends State<_Ruler> {
  double scale = 1.0;
  @override Widget build(BuildContext c) {
    final w = MediaQuery.of(c).size.width - 24;
    final cmCount = (w / (38 * scale)).floor();
    return Column(children: [
      Container(height: 70, decoration: BoxDecoration(color: const Color(0xFFF5F0E0),
        borderRadius: BorderRadius.circular(8)),
        child: CustomPaint(size: Size(w, 70), painter: _RulerPainter(38 * scale))),
      Row(children: [
        const Text('校准', style: TextStyle(fontSize: 10, color: Colors.grey)),
        Expanded(child: Slider(value: scale, min: 0.8, max: 1.2, onChanged: (v) => setState(() => scale = v))),
        Text('x${scale.toStringAsFixed(2)}', style: const TextStyle(fontSize: 10, color: Colors.grey)),
      ]),
