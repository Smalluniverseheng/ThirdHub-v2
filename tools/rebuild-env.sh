...
if [ ! -d /tmp/android/platforms/android-35 ]; then
  ...
  yes | /tmp/android/cmdline-tools/latest/bin/sdkmanager --sdk_root=/tmp/android \
    "platforms;android-34" "platforms;android-35" "platforms;android-36" "build-tools;35.0.0" "ndk;28.2.13676358" "cmake;3.22.1" || true
fi
export ANDROID_HOME=/tmp/android

# ── 构建副本 + 低内存 Gradle 配置（3GB cgroup 沙箱：SerialGC+堆主动收缩+Kotlin进编译进程内）──
echo "== build copy =="
rm -rf /tmp/build_app
...
org.gradle.jvmargs=-Xms256m -Xmx1408m -XX:MaxMetaspaceSize=320m -XX:+UseSerialGC -XX:MinHeapFreeRatio=10 -XX:MaxHeapFreeRatio=25 -Dfile.encoding=UTF-8
