...
> **★ 最新一轮（第十四轮）**：**4.51.0 与 4.52.0 都已四通道发布 + 六项独立校验全 PASS** ——
> 4.51.0 构建源 `a8de5dbc`（CI `build-apk #74`，13 步全 success），APK sha256 `c97e6e41…`；
> 4.52.0 构建源 `d831ee7d`（CI `build-apk #75`，13 步全 success），APK sha256 `aa5409ca…`。
> **★ 三版体积完全相同（29869450 B）** —— 版本串等长把改动字节抵消，**体积对识别版本毫无用处，只认 digest**。
> changelog 已上传（public 54 / admin 369，`latest` 均 4.52.0）。内容见 §6 #23 / #24。

| 部件 | 版本 | 状态 | 下载通道 |
|---|---|---|---|
| V4 前端 APK | **4.52.0 / 50539** | ✅ **第十四轮已四通道发布 + 六项独立校验全 PASS**。APK **29869450 B** / sha256 `aa5409cac55c33bcf01c17735ed86f1d52c29e99ab26f044d3b29905cf3d6430`（包内 `aapt2` = `com.thirdhub.app` / 50539 / 4.52.0；签名 `9b67f322…` **与 4.49.0/4.50.0/4.51.0 同一把钥匙**，可覆盖安装）。构建源 `d831ee7d`（4 files），CI `build-apk #75`（run id `36088795626`，**13 步全 success**）→ `_publish_v4450.mjs` **四通道 PASS** → `_up_changelog.mjs` 200 → `_verify_4520.mjs` **六项 PASS**（含版本化包与别名两处**真下载回算 sha256**、notes 正/反双向串版防护、下一版 `v4.53.0` 仍 404）。内容：阅读界面底栏 8 格→5 格 + 「更多」面板 + **听书改可拖动悬浮球** | Supabase `latest-app.json` + 桶三通道 + Release `v4.52.0` |
| ─ 上一版 V4 APK | **4.51.0 / 50538** | ✅ **第十四轮已四通道发布 + 六项独立校验全 PASS**。sha256 `c97e6e41709d3a6845e38d30625aecd6df343debea03f9cc2fc3d2649f070a75`（29869450 B）。构建源 `a8de5dbc`（`build-apk #74`，run id `36087970557`，13 步全 success）。内容：**模块间左右滑动停用** / 搜索方块停止键 + 确认 / 结果一条条蹦出且一直拉到底 / 详情页标签·来源·换源·简介展开 / 目录默认 30 章懒加载 / 搜索列表副标题补标签与来源 | Supabase `latest-app.json` + 桶三通道 + Release `v4.51.0` |
...
| 23 | **★ 用户点名的「模块间左右滑动」定案并落地**（第十四轮） | ✅ **已随 4.51.0 发布** | 用户原话：「各个模块之间不能左右滑动，切换到别的模块，这个功能应该是最优先级，**你怎么也没有修好**？」<br>**真病根不是手势算法不够好，而是把需求理解反了**：4.44.0 实现的 `NavSwipeRecognizer` 是**启用**并**默认打开**左右滑动切模块，而 2026-09-24 的日志里留着用户原话「每一次滑动都是独立的模块，**不要让左右滑动变成左右切换模块**」并标注「**严不允许**」—— 祈使句否定式，要的是**禁止**。方向反了，再怎么调判定阈值都不可能"修好"。<br>**修法是三件事同时做，缺一条就等于「改了但没生效」**：① 导航层不注册任何横向识别器（删 `import 'core/nav_swipe.dart'`、删 `_swHand/_swFrom/_swBasePx` 字段与 `_swDragTo()/_swSettle()`、`RawGestureDetector` → 直接 `child: pageView`）；② `AppSettings.init()` 里 `await enforceNavSwipeOff()` **幂等清盘**（否则老安装盘里那条 `nav_swipe=true` 还在，继续能滑；且必须**早于任何 sync/上传**，不然本机 true 会先同步到别的端）；③ 开关在 UI 上**只读**、`setNavSwipe(v)` **恒写 false**（参数有意忽略），设置页从 `SwitchListTile` 降级为 `ListTile + trailing: Text('已关闭')`。<br>**边界**：各模块**内部**的横向手势（封面翻页 / 分段行横滑 / 阅读器左右翻页 / 歌词滚动）完全不受影响 —— 滑动只在本模块内生效、永不溢出成切模块，这正是用户说的「每一次滑动都是独立的模块」。<br>**验证**：`nav_swipe_selfcheck` 57 条仍全 PASS（纯函数与存档文件保留、不删）；包内三向探针阳性 7 条（含 `nav_swipe = false（用户已硬性关闭`）+ 阴性 2 条（旧文案 `在正文区横向滑动即可换模块` 在 4.51.0 已消失）+ 对照 3 条 **12/12 PASS** | — |
| 24 | **用户点名的另外 5 件事 + 阅读界面**（第十四轮） | ✅ **已发布（4.51.0 五项 + 4.52.0 阅读界面）** | 用户原话逐条对应：<br>①「引擎收了几百几千本，但前端只收到了二十几本」→ **两处叠加的根因**：首屏只取 `page=1`（40 条）后不自续，且渲染窗口 `_shown` 初始 40 + `all.take(_shown)` ⇒ **数据其实已在内存里，只是没被画出来**。新增 `_autoPullLoop()`（自动连续翻页，四个退出条件：用户停止 / 搜索代次 `_seq` 变化 / 引擎无更多 / 组件卸载）+ `_pumpShown()`（逐帧 +40 条撑开渲染窗口，**同时**解决"一条条蹦出来"的观感与"一次构建上千 ListTile 卡死首帧"）。<br>②「前端搜索没有暂停键…应该变成一个正方形的形状，点一下有确认停止」→ 搜索中搜索键变红色方块停止键，点击**先弹确认**（明示已收到多少条 / 引擎共多少条），确认后 `_seq++` 作废在途请求 + 停续拉，**已收到结果全部保留**（停止＝不再拉，不是清空）。<br>③「目录不应该是直接展开的」+「前端和引擎之间丢失了太多信息」→ 详情页补 标签行 / 真实来源名 / **换源**（按书名回引擎重搜，列同书其它来源；**在客户端做，不加引擎端点** —— 协议只增不减）/ 简介 4 行+展开；目录默认 30 章 + 展开全部，并把 `ListView(children:[...])`（**非懒加载**，最长一本书 1926 章会卡死首帧）换成 `CustomScrollView + SliverList.builder`（本仓首次使用 Sliver*）；搜索列表副标题由只显 author 改为 `author · kind · sourceName`（缺的跳过）。<br>④ 阅读界面（4.52.0）：底栏 8 格 → **5 格**（目录/书签/搜索/夜间/更多）+ **「更多」面板**收纳次要项 + **听书改成可四处拖动的悬浮球**（位置**归一化**持久化、松手贴边、按状态换图标、菜单收起时半透明），并去掉设置面板里的重复项（一个设置只有一个入口）。<br>**验证**：包内三向探针 12/12 + 10/10；八连 **PASS 1195 / FAIL 0** 零回归；`dart format --output=none` 语法闸门（**真的抓到过一处编译级错误**：`showModalBottomSheet` 嵌套少一层括号）| — |
| 25 | **发版核对脚本「只查上一版」的缺口**（第十四轮新发现并修） | ✅ **已修** | 第十三轮的 `_state_now_13.cjs` 只回读了 `v4.49.0`、**漏了刚发的 4.50.0** → 会让人误以为 4.50.0 的 Release 通道没发（四通道其实都在）。**漏掉的恰恰是最需要确认的那一版**。已新写 `_state_now_15.cjs`：① 版本常量写在文件顶部显式宣告（`VER`/`PREV` 走命令行参数）；② **自动把最近 20 个 tag 全列出来**而不是逐个手写；③ 末尾打印「本脚本本轮覆盖到哪个 tag」+ 自动判「在途版是否已进 Release（危险）/ 上一版是否缺失（四通道有缺口）」，让缺口自己暴露 | 铁律：**改了发版脚本，就要顺手补脚本自己的盲区** |
| 26 | **★ 库模式搜索「不像一条一条蹦出来」**（第十五轮，用户点名） | ✅ **完成（模拟器验证已收口 + 第二十二轮修复批次）** | 用户原话：「你这个搜索不像是一条一条蹦出来的呀」。**根因**：库模式（`agg` 资源库聚合结果）在 build 里一次性 `for` 全量渲染，`_pumpShown()` 只看 `engItems` ⇒ 库模式根本不在逐条通道里；且首帧 `_shown = _pageSize(40)` 使首批 40 条也是一次全出。**修法（统一逐条通道）**：① 新增 `_AggRow` 把分组头（源名+latency）与条目**摊平成线性流水**，按 `_shown` 窗口 `.take(_shown)` 截断渲染；② `_renderTotal`（agg→`_aggRowCount`，否则 `engItems.length`）+ `_pumpStepFor(total)` 自适应步长（≈1 秒铺完 = total/60 帧，clamp 到 1..`litePumpStep`）；③ 首帧窗口 `_firstShow = 6`，之后逐帧撑开。**附带修掉四个真缺陷**：`typeNames` 是 `static final` **冻结译文**（切语言不更新 → 改 getter）；库模式明明有结果却顶着「没有找到相关内容」假空态（`go()` 恒置 `engItems=[]` 而空态无条件看它）；单类型缺空态、多类空态漏判 `musics`；库模式 `Api.get` 无代次守卫（补 `mySeq != _seq`）。**工程闸门两处**：`changelog_selfcheck.dart` 正向匹配改**容忍空白正则**（`dart format` 把注册表折行后单行 `contains` 必失配 —— 自检自身缺陷）；`_sync_check.mjs` 的 `INCLUDE_DIRS` 补 `flutter_app/tool`（原盲区 ⇒ `i18n_selfcheck.dart` 差点复现 segLabel「改了没推」事故，待推 4→6 一次推齐）。E1 EnginePlugin 对账由「未完成」转**完成**（`test_engine_plugin.cjs` 21/0）。**验证**：九连自检 **PASS 1212 / FAIL 0** 零回归；CI `#11 dart-selfcheck` success @ `5f8955c`；x64 测试包 `#84` 构建中 → **模拟器验证收口（2026-09-26，emulator-5554）五项全过**：①「一条条蹦出」库模式流水上屏实测；②typeNames 切语言即时更新（fr↔zh 同进程互切全树汉化、无重启，pref 同步落盘）；③segLabel 单行（法语小说模块 4 段等高 126px，像素带谱恰 1 行带）；④暂停键全流程（搜索中变「停止」方块→确认弹窗 4 次稳定复现→**冻结对照裁决**：停止版冻 0 条 6s vs 对照组同词自然 5 条，`_seq` 代次守卫真丢弃在途批次；「已收保留」15 条、「可续搜」均实证）；⑤模块间左右滑动已禁止（行为+代码 `navSwipe=>false` 硬编码+prefs 三方一致）。**验证揪出 3 类真缺陷 → 第二十二轮一次修完**（commit `605efaae`）：a. 引导页 14 键字典双缺（"第十六轮注释称已补译文"实为假完成）→ 补 6 语言译文；b. `_askStop()` 计数源读 `engItems` 库模式恒 0 → 改读 `_aggCount(agg)`（**后续深挖发现单改计数源不够**：弹窗文本在打开那刻算死，而弹窗常开在请求途中、`_pumpShown` 在背后继续铺屏 —— 「弹窗 0 条、屏上 15 条」的假象根因是**文本冻结**，见 commit `e37b7d3d` 实时计数修复）；c. 验证路径 ~19 处裸中文不过 tr() → 56 键过 `tr(k).replaceAll('{{n}}'/'{{m}}')` + `i18n_extra` 6 语言各补 56 条（~260 键/语言）。修复批次闸门：九连 **PASS 1212 / FAIL 0**、i18n 覆盖 **80→128 键 × 6 语言零回退**、`_sync_check` 待推 0。第二批（commit `d4d99440`）：AccountTile 6 键（登录 / 注册×2、ThirdHub 账号 · 与网页版同一体系、没有账号？点这里注册、当前为游客模式…、已通过{{n}}自动连接后端）模拟器复验全法语。**停止弹窗真修复（commit `e37b7d3d`）**：计数改「分模式 + 弹窗开着期间 120ms tick 实时重算」（ValueListenableBuilder），条目一到弹窗数字跟着涨，模拟器实测弹窗计数随流入跳数。残留 482 处裸字面量存量（非验证路径，初版 492 核销第二批 10 行）见 `docs/i18n-裸字面量存量清单.md` | — |

> **第十四轮新增的硬闸门（别再靠人眼）**：
> - **包内三向探针**（`_probe/_apk_probe_4510.py` / `_apk_probe_4520.py`）：阳性（新版有/旧版无）+ 阴性（新版无/旧版有）+ 对照（两版都有）**三向同时过**才算证据。
>   只验「新版里有 X」是**弱证据** —— 有可能 X 本来就在旧版里。本轮实测 **12/12** 与 **10/10** 全 PASS。
> - **★ 三版体积完全相同（4.50.0 / 4.51.0 / 4.52.0 都是 29869450 B）** ⇒ **体积对识别版本毫无用处，只认 digest**。
>   这条已是第三次应验（4.49.0 两个 run 体积也相同）。
> - **中文探针必须 utf-8 + utf-16le 两种编码各搜一遍**（Dart AOT 对非 Latin-1 串按 **UTF-16** 存，按 UTF-8 搜**必然 0 命中** → 会误判成"功能没进包"）。
> - **阴性探针往往找不出第二条**：本轮多数改动是**搬家**（底栏标签移进「更多」）而不是删除，标签串仍在同一份 `libapp.so` 里 ⇒ 「按串搜」只能证明新增、不能证明移除。**「改了」不等于「删了」**。
> - **`_publish_v4450.mjs` 的 notes 串版防护按版本登记关键词**（`KEYWORDS` 表），本轮补 `'4.52.0': /更多|悬浮球|底栏/`。此前只对 4.45.0 特判、其余 `/./` 等于"非空即过"。
> - **`_verify_45xx.mjs` 的 `nextTag` 必须跟着改**（4.51.0 那版写的是 `v4.52.0`，4.52.0 版要写 `v4.53.0`），否则第 ⑥ 通道会自证失败。
