...
## 六、版本与前向兼容(v1.1 新增, 所有实现必须遵守)

**核心原则: 新增引擎/新能力 = 只加 caps 与字段, 永远不改已有字段的含义与结构。**

1. **协议版本**: `/thp/meta` 返回 `"protocol": "THP/1.x"`; 次版本号只增不减。实现方按主版本号 "THP/1" 判断兼容, 忽略次版本细节。
2. **能力协商(caps)**: 引擎能力全部通过 caps 字符串声明(`novel,comic,video,music,audio,live,discover`)。**未来新型引擎(如 game/rss)直接广播新 cap 即可接入, 前后端无需改协议**——未知 cap 一律忽略并展示为"更多能力"。
3. **未知字段容忍**: 任何 JSON 响应里出现实现方不认识的字段, 必须原样忽略, 不得报错; 转发时尽量透传。
4. **可选端点声明**: 除 meta/search/chapters/content 四个基础端点外, 其他端点(如 `/thp/discover`)均为可选, 引擎在 caps 里声明 `discover` 才表示支持; 调用方遇到 HTTP 404 或 `{"object":"error"}` 必须自行降级(前端发现页降级为热词搜索)。
5. **数据类型扩展**: `content` 的 `type` 允许新增取值; 调用方按已知类型渲染, 未知类型显示"该类型暂不支持预览"并允许用浏览器打开原始地址。
6. **第三方接入**: 任何开发者按本文档实现引擎即可被前后端自动发现与调用(广播即接入, 无注册无审核); 按同一协议也可以实现替代后端/替代前端——协议是生态唯一的约定。
7. **错误归一**: `{"object":"error","data":{"message":"...","code":"..."}}`; code 建议值: `UPSTREAM_FAIL / NOT_FOUND / UNSUPPORTED / RATE_LIMIT`, 自定义 code 允许, 调用方只依赖 object==error 判断失败。
