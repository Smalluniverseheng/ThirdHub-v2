...
- 后端 /v1/novel/* 路由优先走 Legado 设备, 无设备时 404(今晚不做内置降级)
- 前端: 现有网页版搜索/阅读页对接(不改 UI, 只换数据源)

## 4. 今晚验收脚本（开发者照做）

1. 安卓手机: 装官方 Legado → 设置→Web服务→启动(记端口1122)
2. 同手机: Termux 跑 install-android.sh → 看到"后端就绪 https://192.168.x.x:9527 指纹xxxx"
3. 浏览器开该地址 → 首次连接确认指纹 → 书源列表出现"Legado(局域网)"
4. 搜一本书 → 点开 → 翻三章 → ★今晚通关★
