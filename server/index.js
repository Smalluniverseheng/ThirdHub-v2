...
try {
  ...
  udp.on('message', (msg, rinfo) => {
    const m = String(msg).match(/^THP\/1 HELLO (\d+) ([\w,\-]+)$/);
    if (!m) return;
...
const thpOnline = (cap) => devices.filter(d => d.device_type === 'thp' && (Date.now() - (d.last_seen || 0)) < 90000
  && (!cap || (d.caps || []).includes(cap)));
...
async function handle(req, res, body) {
  ...
  if (p === '/v1/library/delete' && req.method === 'POST') {
    const d = JSON.parse(body || '{}');
    library = library.filter(b => b.id !== d.id); saveLib(library);
